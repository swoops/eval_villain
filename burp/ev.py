...
defconfig = {
    ...
    "functions": [
        "eval", "setter(innerHTML)", "setter(outerHTML)", "document.write", "document.writeln"
    ],
    "types": ["string"],
}
...
class BurpExtender(IBurpExtender, IProxyListener):
    config = None
    clparse = re.compile(r'^Content-Length:\s*(\d+)\s*$')
    confdir = os.path.expanduser("~/.config/eval_villain_burp/")
    conffile = confdir + "config.json"

    def registerExtenderCallbacks(self, callbacks):
        callbacks.setExtensionName('Eval Villain for Burp')
        callbacks.registerProxyListener(self);
        self.config = self.getconfig()
        print("loaded")
        return

    def processProxyMessage(self, message_req, message):
        if message_req: return
        msg = message.getMessageInfo()
        refid = message.getMessageReference()
        s = self.add_script(msg.getResponse().tostring(), refid)

        if s:
            print("replacing msg: %d" % refid)
            msg.setResponse(bytearray(s))

    def getscript(self):
        global switcheroo
        script = "<script>\n"
        script += switcheroo
        script += json.dumps(self.getconfig())
        script += ");\n</script>"
        return script
        

    def add_script(self, s, refid):
        # process headers
        content_len = 0
        content_type = False
        for i in s.split("\n"):
            if i.startswith("Content-Type:"):
                if i.startswith("Content-Type: text/html"):
                    content_type = True
                else:
                    return None
                if content_len:
        ...
        for i in ["<head>", "<html>"]:
            if i in s:
                ...
                replace_str = i + script
                p = self.clparse.findall(content_len)
                if not p: 
                    return None
                oldlen = int(p[0])
                newlen = oldlen + len(script)
                return s.replace(i, replace_str, 1).replace(content_len, "Content-Length: %d\r" % newlen, 1)

        print("could not find header for %d" % refid)
        return None

    def _keysmatch(self, d, l):
        keys = d.keys()
        skeys = set(d.keys)
        if skeys != set(l) or len(keys) != len(skeys):
            return False


    def validate_config(self, conf):
        if not self._keysmatch(conf, ['needles', 'functions', 'formats', 'types', 'blacklist']):
            return False
        forms = conf["formats"]
        if not self._keysmatch(forms, ['title', 'interesting', 'args', 'needle', 'query', 'fragment', 'winname', 'cookie', 'localStore', 'stack']):
            return False
        for i in forms:
            if not _keysmatch(forms[i], ['pretty', 'use', 'open', 'default', 'highlight']):
                return False
    ...
    def getconfig(self):
        ...
        with open(self.conffile, "w") as fp:
            json.dump(defconfig, fp)
            print("Created config file at: %s" % self.conffile)
